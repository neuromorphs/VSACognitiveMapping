...
**Why whitening loses even though its crosstalk number is better: it buys
that number by attenuating signal, not just noise.** `crosstalk_scaling.py`
decomposes raw → centred → zscored → whitened, all through the identical
random-projection FPE encoder, and reports `mean_on` (the recalled signal
amplitude) alongside χ at matched N. Whitening keeps *reducing* χ past the
z-scored point, but `mean_on` keeps falling too — it does not stop at
removing noise, it flattens the low-variance tail directions that still carry
real signal up to the same amplitude as everything else:

| content (crop mean descriptor, N=2,429) | zscored χ | whitened χ | zscored `mean_on` | whitened `mean_on` | signal lost |
|---|---|---|---|---|---|
| yolov8n | 37.71 | 9.11 | 0.0349 | 0.0138 | **−60%** |
| resnet50 | 24.54 | 9.83 | 0.0262 | 0.0165 | **−37%** |
| dinov2 | 45.71 | 9.87 | 0.0443 | 0.0160 | **−64%** |
| yolov8n (whole-frame, N=1,239) | 16.33 | 3.81 | 0.0387 | 0.0151 | **−61%** |

χ keeps improving 2–5× from zscored to whitened, but retrieval is a function
of signal *against* interference, not interference alone — losing 37–64% of
`mean_on` to buy a further crosstalk cut is the mechanism behind the
monotonic retrieval degradation above, not a separate effect. ResNet-50 loses
the least signal to whitening (−37%), consistent with it already having the
lowest raw anisotropy of the trained encoders (§4) — there is less low-variance
tail for whitening to inflate.
...
4. ~~**Is the crosstalk multiplier mostly a mean-removal effect?**~~ **Answered.**
   Centring alone recovers roughly half of the *log*-gain: on yolov8n crops,
   raw→centred is 4.2× of the total raw→whitened 19.9×, and on the whole-frame
   variant 4.0× of 22.3× — about 45–48% of `log(gain)` from mean removal alone,
   the rest from the rotation/rescaling step. Neither ingredient dominates
   outright; quote whitening gains as "centring + spectral" from here on. See
   the signal-cost table in §2.
